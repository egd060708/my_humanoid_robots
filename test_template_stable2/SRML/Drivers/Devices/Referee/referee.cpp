...
else
{
	...
	else compensate_num_pitch_str[0]=' ';
...
void referee_Classdef::Engineer_AutoMode_Update(uint8_t status, uint8_t _layer, uint16_t start_x, uint16_t start_y, uint8_t size)
{
	...
	static uint8_t last_status = status;
		...
		switch(status){
			...
			case 4:
					Mine_Icon(_layer, start_x + size/2, start_y - size/2, size, 2, YELLOW, ADD_PICTURE);break;	//空接
			...
			break;
				...
				case 5:Rescue_Icon(_layer, start_x + size/2, start_y - size/2, size, PINK, ADD_PICTURE);break;	//救援
...
void referee_Classdef::Sentry_Mode_Update(uint8_t mode, uint8_t _layer, uint16_t start_x, uint16_t start_y, uint16_t size, colorType_e _color)
{
	...
	if(last_mode != mode)
	{
		while(repeat_cnt--)
			switch (mode)
			{
				case 0:Draw_Char(_layer, start_x + 6 * size, start_y, mode_name, auto_str, sizeof(auto_str), size, _color, MODIFY_PICTURE);break;
				case 1:Draw_Char(_layer, start_x + 6 * size, start_y, mode_name, manual_str, sizeof(manual_str), size, _color, MODIFY_PICTURE);break;
			default:
				break;
			}
	}
...
void referee_Classdef::Sentry_Operation_Update(uint8_t opeartion, uint8_t _layer, uint16_t start_x, uint16_t start_y, uint16_t size, colorType_e _color)
{
	...
	if(last_operation != opeartion)
	{
		while(repeat_cnt--)
			switch (opeartion)
			{
				case 7:Draw_Char(_layer, start_x + 11 * size, start_y, operation_name, notify_hightwoer_str, sizeof(notify_hightwoer_str), size, _color, MODIFY_PICTURE);break;
				case 6:Draw_Char(_layer, start_x + 11 * size, start_y, operation_name, outpost_str, sizeof(outpost_str), size, _color, MODIFY_PICTURE);break;
				case 5:Draw_Char(_layer, start_x + 11 * size, start_y, operation_name, road_str, sizeof(road_str), size, _color, MODIFY_PICTURE);break;
				case 4:Draw_Char(_layer, start_x + 11 * size, start_y, operation_name, stop_str, sizeof(stop_str), size, _color, MODIFY_PICTURE);break;
				case 2:Draw_Char(_layer, start_x + 6 * size, start_y, operation_name, back_str, sizeof(back_str), size, _color, MODIFY_PICTURE);break;
				case 1:Draw_Char(_layer, start_x + 11 * size, start_y, operation_name, outeng_str, sizeof(outeng_str), size, _color, MODIFY_PICTURE);break;
				case 0:Draw_Char(_layer, start_x + 11 * size, start_y, operation_name, defalut_str, sizeof(defalut_str), size, _color, MODIFY_PICTURE);break;
