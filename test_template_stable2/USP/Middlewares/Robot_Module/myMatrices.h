...
namespace mM {
    ...
    template <class T>
    inline myMatrices<T> kron(myMatrices<T>& A, myMatrices<T>& B) {
    ...
    template <class T, class... Args>
    inline myMatrices<T> kron(myMatrices<T>& A, myMatrices<T>& B, Args&... rest) {
        uint16_t m = A.getRows();
        uint16_t n = A.getCols();
        uint16_t p = B.getRows();
        uint16_t q = B.getCols();
    ...
    /* 构建对角块矩阵 */
    template <class T>
    inline myMatrices<T> blkdiag(const myMatrices<T>& A, const myMatrices<T>& B) {
    ...
    template <class T, class... Args>
    inline myMatrices<T> blkdiag(const myMatrices<T>& A, const myMatrices<T>& B, const Args&... rest) {
    ...
    /* 创建零矩阵 */
    template<class T>
    inline myMatrices<T> zeros(uint16_t n)
    {
        myMatrices<T> result(n, n);
        return result;
    }
    template<class T>
    inline myMatrices<T> zeros(uint16_t rows, uint16_t cols)
    {
        myMatrices<T> result(rows, cols);
        return result;
    }
    ...
    /* 创建单位对角阵 */
    template<class T>
    inline myMatrices<T> eye(uint16_t n)
    {
        myMatrices<T> result(n, n);
    ...
    /* 矩阵行向合并（行不变列变） */
    template<class T>
    inline myMatrices<T> rowCombine(myMatrices<T>& A,myMatrices<T>& B)
    {
    ...
    template <class T, class... Args>
    inline myMatrices<T> rowCombine(myMatrices<T>& A, myMatrices<T>& B,Args... rest)
    {
        myMatrices<T> result(A.getRows() + B.getRows(), A.getCols());
        if (A.getCols() != B.getCols()) {
            throw  std::invalid_argument("Matrix dimensions are not compatible for combine");
    ...
    /* 矩阵列向合并（行变列不变） */
    template<class T>
    inline myMatrices<T> colCombine(myMatrices<T>& A, myMatrices<T>& B)
    {
    ...
    template<class T, class... Args>
    inline myMatrices<T> colCombine(myMatrices<T>& A, myMatrices<T>& B, Args... rest)
    {
        myMatrices<T> result(A.getRows(), A.getCols() + B.getCols());
        if (A.getRows() != B.getRows()) {
            throw  std::invalid_argument("Matrix dimensions are not compatible for combine");
