...
namespace
{
     ...
     *
     */
    class MotorAngle : public MultiTurnRecorder
    {
    public:
...
public:
    ...
    float EMFCompensationScale = 55;//反电动势补偿系数
    void setCurrentOut(float target_current)
    {
        this->Out = currentLoop_Kp * (target_current - this->TorqueCurrent) + (EMFCompensationScale * this->speed + target_current) / 0.75f;
    }
private:
    uint8_t temperature = 0;
};
...
namespace motor_dji_send
{
    ...
    */
    ...
    void MotorMsgSend(QueueHandle_t CanQueueHander, MotorType &motor)
    {
        ...
        MotorMsgSend(CanQueueHander, motor_arr);
    }
}
/*
