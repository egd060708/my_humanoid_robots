...
private:
    ...
    const float RATIO_RECEIVE_VOLTAGE = 0.1f;
    const float RATIO_RECEIVE_ANGLE = 0.01f;
    const float RATIO_ENCODER_TO_ANGLE = 360 / 65536.f;
    float RATIO_CURRENT_TO_A = 16.5f / 2048.f;// 小型号16.5，大型号33
    float TORQUE_CONSTANT = 0.32f;

public:
    LkMotorBass(uint8_t _id);
    LkMotorBass(bool id1, bool id2, bool id3);
    ...
    void setTorqueConstant(float _val);
    void setCurrent2ARatio(float _val);
    ...
    void iqOpenControl(float _targetTorque);                            // 开环力矩控制
    void iqOpenControl_Current(int16_t _targetCurrent);                 // 开环力矩控制，输入目标电流值，主要用于速度环在MCU上运算时，向电机下发控制量，但是封库时测试用起来效果并不佳
...
CAN_COB &MotorMsgPack(CAN_COB &motor_msg, LkMotorBass &motor, MotorTypes &...motors)
{
